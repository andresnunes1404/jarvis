...
- **Development-start sub-gate**: when the three checks above all find nothing, `is_development_start_trigger_phrase()` runs — same file, still NO LLM (deterministic substring/exact-match against `_DEVELOPMENT_START_TRIGGER_PHRASES`/`_BARE_DEVELOPMENT_START_PHRASES`). If the user's text deterministically requests starting development on an existing plan, forces `run_tool_with_retries(tool_name="startProjectDevelopment", ...)` directly. Same failure mode as the start-trigger sub-gate but observed one step later in the pipeline: the router correctly selected `startProjectDevelopment`, but the small model narrated "Iniciando o desenvolvimento do projeto..." instead of actually invoking it (no `🛠️ Agent` line in the log). Normal (LLM) routing remains the fallback for phrasing this list doesn't cover. See `project_intake.spec.md` "Starting development".
...
user input
  └─▶ [3c] Project Intake Gate    (no LLM — active session? force projectIntake tool, return, done)
        ...
        └─▶ [3c] Development-start sub-gate  (no LLM — none of the above matched, but "start development"/"delega ao antigravity" phrase? force startProjectDevelopment tool, return, done)
